...
def parse_oecd(path, year = None):
    ...
    oecd_totals_row = ["TLS", "VA", "OUT"]

    tmp_copy_oecd_raw = oecd_raw.copy()

    oecd_raw.drop(oecd_totals_col, axis=1, errors="ignore", inplace=True)

    factor_input = oecd_raw.loc[['TLS', 'VA'],:]
    ...
    out = []
    for ll in Z.index.str.split("_"):
        if len(ll) == 1:
            out.append(tuple([ll[0], '0']))
        elif len(ll) == 2:
            out.append(tuple(ll))
        elif len(ll) == 3:
            out.append(tuple([ll[0], ll[1] + '_' + ll[2]]))
        else:
            raise ValueError('Gotcha!')

    Z_index = pd.MultiIndex.from_tuples(out) #pd.MultiIndex.from_tuples(tuple(ll) for ll in Z.index.str.split("_"))
...
def parse_oecd_old(path, year=None):
    """Parse the OECD ICIO tables
